...
/**
 * Canonical list of spells used for gating.
 * <p>
 * Lookups are O(1) via a precomputed name→enum map to avoid per-call iteration.
 * The map is built once at class-load time and then exposed only through helpers.
 */
@Getter
public enum Spell {
    // STANDARD
    ...
    /**
     * Immutable map of display name → enum for O(1) lookups.
     */
    private static final Map<String, Spell> NAME_TO_SPELL;

    static {
        Map<String, Spell> m = new HashMap<>(values().length * 2);
        for (Spell s : values()) {
            m.put(s.getSpellName(), s);
        }
        NAME_TO_SPELL = Collections.unmodifiableMap(m);
    }
    ...
    /**
     * @return true if {@code target} is an exact spell display name.
     */
    public static boolean isSpell(String target) {
        return target != null && NAME_TO_SPELL.containsKey(target);
    }
}
