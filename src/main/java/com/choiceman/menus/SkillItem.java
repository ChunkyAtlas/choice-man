...
/**
 * Catalog of item → skill-operation mappings used to gate actions.
 * Some items require an unlock before enabling their {@link SkillOp}.
 * <p>
 * Lookups are via a single unmodifiable id→enum map to avoid duplicate structures
 * and accidental mutation. Using unmodifiable views is an established Java best
 * practice for defensive design. See Oracle docs on unmodifiable collections.
 */
@Getter
public enum SkillItem {
    ...
    /**
     * Fast id→enum lookup.
     * Built once, then exposed as an unmodifiable view to prevent accidental mutation.
     */
    private static final Map<Integer, SkillItem> BY_ID;

    static {
        Map<Integer, SkillItem> m = new HashMap<>();
        for (SkillItem s : values()) {
            m.put(s.id, s);
        }
        BY_ID = Collections.unmodifiableMap(m); // unmodifiable view recommended by Oracle docs
    }
    ...
    /**
     * @return true if {@code id} maps to a defined SkillItem.
     */
    ...
    /**
     * @return the enum for {@code id}, or null if none.
     */
